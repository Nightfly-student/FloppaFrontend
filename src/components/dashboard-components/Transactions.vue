<template>
  <div class="container-xl">
    <button v-if="user.roles.some(e => e.name === 'EMPLOYEE')" :data-bs-target="'#EmployeeSendModal'"
      data-bs-toggle="modal" class="btn btn-primary mb-2">
      Perform transaction
    </button>
    <h1 v-if="transactionsCount == null" class="text-secondary text-center">No transactions found...</h1>
    <div v-else>
      <div class="d-flex mb-2 justify-content-between">
        ...
        <div class="search d-flex gap-2">
          ...
          <input v-if="filterMode == 'amount'" type="number" min="0" max="999999999" class="rounded my-1"
            v-model="amount" @change="offset = 0; currentPage = 1;" />
        ...
        <nav aria-label="...">
          <ul class="pagination">
            ...
            <li v-else class="page-item">
              <button class="page-link" @click="changeOffset(-limit, transactionsCount, false, currentPage, false);
loadTransactions();
              ">
                Previous
              </button>
            ...
            <li v-if="currentPage > 1" class="page-item">
              <button class="page-link" @click="changeOffset(limit, transactionsCount, false, 1, true);
loadTransactions();
              ">
                1
              </button>
